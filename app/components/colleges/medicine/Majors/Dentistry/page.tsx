...
export default function AccountingPage() {
  ...
  useEffect(() => {
    const fetchProgramData = async () => {
      try {
        setLoading(true);
        // استخدام slug=accounting لجلب بيانات تخصص المحاسبة
        const response = await fetch(`/api/api_pages/academic-programs?slug=accounting`);
  ...
  if (error || !programData) {
    return (
      <div className={styles.errorContainer}>
        ...
        <Link href="/colleges/business">
          <button className={styles.ctaButton}>العودة إلى كلية العلوم الإدارية</button>
        </Link>
      </div>
    );
  }
  ...
  return (
    <>
      ...
      <section 
        ...
        style={{
          backgroundImage: `url('${program.hero_image_url || '/images/accounting-hero.jpg'}')`,
          ...
          <div className={styles.projectsGrid}>
            {projects.map((project) => (
              <div key={project.id} className={styles.projectCard}>
                <img 
                  src={project.image_url || '/images/default-project-accounting.jpg'} 
